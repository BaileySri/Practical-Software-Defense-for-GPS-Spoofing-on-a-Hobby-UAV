...
bool SensorConfirmation::confirm(const float a, const float a_err,
  ...
  if (!wrap) {
    ...
    if ((lower <= 0) && (upper >= 360)) {
      ...
      // If the lower value wraps around, confirm if it becomes less than the
      // upper
      lower = fmod(lower, 360);
      if (upper - lower >= 0)
        return true;
    } else if (upper >= 360) {
      // If the upper value wraps around, confirm if it becomes greater than the
      // lower
...
void SensorConfirmation::update() {
  ...
  framework.gpsAvail =
      sensors.currGps.update(AP::gps().get_singleton(), sensors.prevGps);
...
bool SensorConfirmation::run() {
  ...
  if (framework.gpsAvail) {
    ...
    AP_Logger::get_singleton()->Write_CNFR(
        ...
        ToDeg(sensors.currGyro.Error *
              (sensors.currGyro.TimeContained / (float)1000000)));
  ...
  if (framework.ofAvail) {
    AP_Logger::get_singleton()->Write_ACO(
        sensors.pOF.VelNE, sensors.pOF.Err, sensors.cOF.VelNE, sensors.cOF.Err,
        sensors.cOFAccel.Velocity, sensors.cOFAccel.Error);
    res &= AccOF();
...
float SensorConfirmation::NetGpsLimit() const {
  ...
  ret = MAX(0.0F, 2 * (sensors.currGps.Hacc / 0.2F) + sensors.currAccel.Error -
                      std::abs(currAccel - currGps));

  // GpsOF limit
  ret = MIN(ret, (sensors.currGps.Hacc / 0.2F) + sensors.currOF.Err.length() -
                     std::abs(currOF - currGpsNE));
...
float SensorConfirmation::NetOFLimit() const {
  ...
  ret = MAX(0.0F, sensors.cOF.Err.length() +
                      (sensors.cOFAccel.Error * 1.414 / 1.73) -
                      std::abs(currAccelNE - cOF));

  // GpsOF limit
  ret = MIN(ret, (sensors.currGps.Hacc / 0.2F) + sensors.currOF.Err.length() -
                     std::abs(currGpsNE - currOF));
...
bool SensorConfirmation::AccGPS() {
  ...
  return (confirm(dAccVel, sensors.currAccel.Error * sqrtf(3.0F),
                  dGpsVel.length(),
                  (sensors.currGps.Hacc + sensors.prevGps.Hacc) / 0.2F, false));
}
...
bool SensorConfirmation::AccOF() {
  ...
  return (confirm(dAccVel, sensors.cOFAccel.Error * sqrtf(2.0F),
                  dOFVel.length(),
                  sensors.cOF.Err.length() + sensors.pOF.Err.length(), false));
}
...
bool SensorConfirmation::GpsOF() {
  ...
  return (confirm(sensors.currOF.VelNE.length(), sensors.currOF.Err.length(),
                  GpsNE.length(), sqrtf(2.0F) * sensors.currGps.Hacc / 0.2F,
                  false));
}
...
bool SensorConfirmation::GpsMagGC() {
  ...
  // GPS and GPS Error
  std::vector<float> gps{sensors.currGps.Airspeed[0],
                         sensors.currGps.Airspeed[1]};
  float dot = gps[0];  // N * 1 + E * 0
  float det = -gps[1]; // N * 0 - E * 1
  float GpsGC = atan2f(det, dot);
  std::vector<float> ErrGps{
      std::abs(sensors.currGps.Airspeed[0]) -
          std::abs(sensors.currGps.Hacc / (GPS_RATE / (float)1000)),
      std::abs(sensors.currGps.Airspeed[1]) +
          std::abs(sensors.currGps.Hacc / (GPS_RATE / (float)1000))};
  gps[0] = std::abs(gps[0]);
  gps[1] = std::abs(gps[1]);
  ...
  return (confirm(GpsGC, ErrGpsGC, MagGC,
                  ToDeg(sensors.currGyro.Error *
                        (sensors.currGyro.TimeContained / (float)1000000)),
                  true));
}
...
bool SensorConfirmation::GpsOFGC() {
  ...
  mOF[0] = std::abs(mOF[0]);
  mOF[1] = std::abs(mOF[1]);
  dot = mOF[0] * std::abs(sensors.currOF.Err[0]) +
        mOF[1] * std::abs(sensors.currOF.Err[1]); // N * N + E * E
  det = mOF[0] * std::abs(sensors.currOF.Err[1]) -
        mOF[1] * std::abs(sensors.currOF.Err[0]); // N * E - E * N
...
bool SensorConfirmation::AccGpsGC() {
  ...
  std::vector<float> Acc{sensors.currAccel.Velocity[0],
                         sensors.currAccel.Velocity[1]};
  dot = Acc[0];
  det = -Acc[1];
  float AccGC = atan2f(det, dot);
  Acc[0] = std::abs(Acc[0]);
  Acc[1] = std::abs(Acc[1]);
  dot = Acc[0] * std::abs(sensors.currAccel.Error) +
        Acc[1] * std::abs(sensors.currAccel.Error); // N * N + E * E
  det = Acc[0] * std::abs(sensors.currAccel.Error) -
        Acc[1] * std::abs(sensors.currAccel.Error); // N * E - E * N
...
bool SensorConfirmation::AccOFGC() {
  ...
  // Accelerometer and Accelerometer Error
  std::vector<float> Acc{sensors.cOFAccel.Velocity[0],
                         sensors.cOFAccel.Velocity[1]};
  float dot = Acc[0];  // N * 1 + E * 0
  float det = -Acc[1]; // N * 0 - E * 1
  float AccGC = atan2f(det, dot);
  Acc[0] = std::abs(Acc[0]);
  Acc[1] = std::abs(Acc[1]);
  dot = Acc[0] * std::abs(sensors.cOFAccel.Error) +
        Acc[1] * std::abs(sensors.cOFAccel.Error); // N * N + E * E
  det = Acc[0] * std::abs(sensors.cOFAccel.Error) -
        Acc[1] * std::abs(sensors.cOFAccel.Error); // N * E - E * N
  float ErrAccGC = ToDeg(atan2f(det, dot));
  ...
  std::vector<float> ErrOF{
      std::abs(sensors.cOF.VelNE[0]) - std::abs(sensors.cOF.Err[0]),
